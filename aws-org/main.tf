...
# Attach AWS SCP to AWS Organization root, ou, or account
...
data "aws_iam_policy_document" "resource_policy_validation" {
  for_each                = var.resource_policies
  source_policy_documents = [file("resource_policies/${each.key}.json")]
}

# Create AWS Resource Control Policy
resource "aws_organizations_policy" "org_rcp" {
  for_each    = var.resource_policies
  name        = each.key
  description = each.value.description
  type        = "RESOURCE_CONTROL_POLICY"
  content     = data.aws_iam_policy_document.resource_policy_validation[each.key].json
  tags = merge(
    var.defaultTags,
    var.custom_tags
  )
...
# Attach AWS RCP to AWS Organization root, ou, or account
resource "aws_organizations_policy_attachment" "org_rcp_attachment" {
  for_each  = { for rcp in local.rcp_attachment_map : "${rcp.name}_${rcp.target_id}" => rcp }
  policy_id = aws_organizations_policy.org_rcp[each.value.name].id
  target_id = each.value.target_id == "root" ? aws_organizations_organization.org.roots[0].id : each.value.target_id == can(regex("^[0-9]", each.value.target_id)) ? aws_organizations_account.accounts[each.value.target_id].id : aws_organizations_organizational_unit.org_ous[each.value.target_id].id
}
